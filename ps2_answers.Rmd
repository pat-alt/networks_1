...
Noting that we need to have $(\mathbf{T}^T\mathbf{r})^T\mathbf{1}=1$ we have for the steady-state PageRank vector: $\mathbf{r}^T=\frac{1}{10}(3,1,2,1,3)$, where the first entry corresponds to node $A$ and the last to node $E$.
...
d_out <- diag(crossprod(t(G)))
T_20 <- G / d_out
T_lat <- matrix2latex(T_20)
```

Similarly for the network in Figure (\@ref(fig:net20)) we have
...
```{r net20-sol,fig.cap="Derivation of steady-state of network 14.20 in @easley2010networks."}
knitr::include_graphics("www/ps2_1_20_sol.png")
```

and hence $\mathbf{r}^T=\frac{1}{13}(4,2,2,1,2,1)$ for the steady-state PageRank.
...
Finally, for the network in Figure (\@ref(fig:net20)) we have
...
<!-- We can apply this to the three transition matrices: -->

<!-- ```{r} -->
<!-- trans_mats <- list(T_19,T_20,T_21) -->
<!-- output <- rbindlist( -->
<!--   lapply( -->
<!--     1:length(trans_mats), -->
<!--     function(i) { -->
...
Just looking at the network in Figure (\@ref(fig:net-clique)) it is clear that there are two cliques, one at the far left and one at the far right end, both of size 3. 
