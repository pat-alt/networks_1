...
how many paths of length $k ≥ 1$ there are starting at $1$ and ending at $2$?
...
how many cycles of length $k ≥ 1$ there are starting at $1$ and ending at $1$?
...
\begin{aligned}
&& G^k&= \begin{pmatrix}
0 & (n-1)^{( \frac{k-1}{2}-1)} & ... & (n-1)^{( \frac{k-1}{2}-1)} \\
(n-1)^{( \frac{k-1}{2}-1)} & 0 & ... & 0  \\
... & ... & ... & ... \\
(n-1)^{( \frac{k-1}{2}-1)} & 0 & ... & 0
\end{pmatrix}
 \\
...
clust_avg <- transitivity(g, type="average")
n <- nrow(G)
